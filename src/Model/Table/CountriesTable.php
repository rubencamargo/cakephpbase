...
class CountriesTable extends Table
{
    ...
    {
        $validator
            ->requirePresence('num_code', 'create')
            ->notEmptyString('num_code');

        $validator
            ->scalar('iso1_code')
            ->maxLength('iso1_code', 2)
            ->requirePresence('iso1_code', 'create')
            ->notEmptyString('iso1_code');
        ...
        $validator
            ->scalar('name_caps')
            ->maxLength('name_caps', 80)
            ->requirePresence('name_caps', 'create')
            ->notEmptyString('name_caps');

        $validator
            ->scalar('iso3_code')
            ->maxLength('iso3_code', 3)
            ->allowEmptyString('iso3_code');
